    <section class="faq">
            <div class="content">
                <h2 class="mentoria-title">PERGUNTAS FREQUENTES</h2>
                <label class="collapsible" for="collapsible" class="lbl-toggle">
                    <span>Quais os dias e horários das aulas ao vivo?</span>
                    <input id="collapsible" class="toggle" type="checkbox">
                    <p>Todos os sábados a partir de 18 de setembro, de 09:00 às 12:00, no horário de Brasília.</p>
                </label>
                <label class="collapsible" for="collapsible1" class="lbl-toggle">
                    <span>Como será o seu suporte?</span>
                    <input id="collapsible1" class="toggle" type="checkbox">
                    <p>Suporte por telefone, WhatsApp, Telegram e E-mail. O suporte é fornecido diretamente por seu mentor, Pedro Marins.</p>
                </label>
                <label class="collapsible" for="collapsible9" class="lbl-toggle">
                    <span>Quem é Pedro Marins?</span>
                    <input id="collapsible9" class="toggle" type="checkbox">
                    <p>MBA em Gestão de Conhecimento e Negócios pela COPPE/UFRJ, exerce em vários projeto o papel de criador de negócios inovadores. Começou sua carreira como dev front-end e hoje atua como professor da Alura e do Ibmec.
                    <br><br> Depois de trabalhar em uma centena de projetos, montar dezenas de equipes e entrevistar mais de mil pessoas, chegou a hora de mostrar como conquistou suas vagas e como você pode conseguir também.</p>
                </label>
                <label class="collapsible" for="collapsible2" class="lbl-toggle">
                    <span>Por quanto tempo terá acesso às aulas?</span>
                    <input id="collapsible2" class="toggle" type="checkbox">
                    <p>O acesso às aulas é vitalício.</p>
                </label>
                <label class="collapsible" for="collapsible8" class="lbl-toggle">
                    <span>Por quanto tempo terá acesso à comunidade?</span>
                    <input id="collapsible8" class="toggle" type="checkbox">
                    <p>Por 1 ano.</p>
                </label>
                <label class="collapsible" for="collapsible3" class="lbl-toggle">
                    <span>Por quanto tempo terá acesso ao suporte?</span>
                    <input id="collapsible3" class="toggle" type="checkbox">
                    <p>Por 4 meses.</p>
                </label>
                <label class="collapsible" for="collapsible4" class="lbl-toggle">
                    <span>Até que dia/hora é a matrícula?</span>
                    <input id="collapsible4" class="toggle" type="checkbox">
                    <p>Até sexta, 17 de setembro, às 23:59 (horário de Brasília). A mentoria começará no dia 18 de setembro.</p>
                </label>
                <label class="collapsible" for="collapsible5" class="lbl-toggle">
                    <span>Até que dia/hora é possível fazer a matrícula com valor promocional?</span>
                    <input id="collapsible5" class="toggle" type="checkbox">
                    <p>Até quarta, 15 de setembro, às 23:59 (horário de Brasília).</p>
                </label>
                <label class="collapsible" for="collapsible6" class="lbl-toggle">
                    <span> Qual o valor do investimento?</span>
                    <input id="collapsible6" class="toggle" type="checkbox">
                    <p>O investimento no Conquiste Sua Vaga com desconto promocional tem o valor de R$ 997 à vista ou 12 x R$ 99,70.</p>
                </label>
                <label class="collapsible" for="collapsible7" class="lbl-toggle">
                    <span>Como posso pagar?</span>
                    <input id="collapsible7" class="toggle" type="checkbox">
                    <p>Na opção parcelada, você pode pagar com cartão de crédito. Na opção à vista, você pode pagar com cartão, pix, boleto bancário ou PayPal. Se escolher pagar com cartão, você pode utilizar mais de um cartão de crédito.</p>
                </label>
            </div>
        </section>